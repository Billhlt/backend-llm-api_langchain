项目简介
  ...
  基于 LangChain + LangGraph + FastAPI 的 LLM 对话接口服务，演示带记忆的 Agent 编排：通过 create_agent
  构建支持工具调用的 Agent，并用 InMemorySaver 检查点实现按会话（chatId）隔离的对话记忆。
  ...
  代码从环境变量 DEEPSEEK_API_KEY 读取密钥，请先前往 https://platform.deepseek.com 获取你自己的 API Key。
  ...
  - 代码在 Windows / Linux 均可运行，但安装命令需按你的平台选择（Windows 用 PowerShell 的 $env:，Linux 用 export）。
